...
def solve_board_by_hamiltonian_path(board): # wroking method
    ...
    if not hamiltonian_path_util(board, path, 1, found):
        ...
        return path
def hamiltonian_path_util(board, path, pos, found): # recursive part of working method
    if pos == len(board):
        return True

    for cube in get_adjacent_hexes(path[pos - 1], board):
        if cube not in path and ((board[cube] == 0 and pos+1 not in found) or board[cube] == pos+1) and does_path_make_sense(board, path):
            path[pos] = cube

            # Recur to construct the rest of the path
            if hamiltonian_path_util(board, path, pos + 1, found):
                return True
...
"""
    for cube in board.keys():
        if cube not in path:
            allll = True
            for n in get_adjacent_hexes(cube, board):
                if n in board:
                    if n not in path:
                        allll = False
            if allll:
...
"""
    #print(path)
    global groups_col
    groups_col = find_isolated_groups(board, path)
    if len(find_isolated_groups(board, path)) < 2:
        return True

    # how do we know if a group is valid?
    return True
    return False
    """
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_r:  # Reset
                    ...
                    print(board)
                    groups_col = find_isolated_groups(board, [])
